...
Cache.prototype.get = function(key) {
  ...
  if(!data || (data.expireAt && new Date() >= data.expireAt)) {
    return null;
  }
 ...
 *
 * @param {String} Key to identify the stored value.
 * @param {String} Stored value.
 * @param {Object} Pernamency options.
 */
Cache.prototype.set = function(key, value, options) {
  ...
  if(!options.expireAt && options.expire) {
 ...
 *                              of the request.
 */
Cache.prototype.respond = function(method, turl, response) {
  var self = this,
      data = self.get(method + turl.href);
